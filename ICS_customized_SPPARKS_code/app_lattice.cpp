...
AppLattice::AppLattice(SPPARKS *spk, int narg, char **arg) : App(spk,narg,arg)
{
  ...
  dt_diff = 6;   //Once an hour update of recent cancer lysis
  diffCount = 0;
  twindow = 12; // 12 hour memory for T cell proliferation/recruitment dependence on cancer lysis
  nDeadCounti = 0;
  proliferation_update_bool = false; // initiates update of all cd8+ T cell proliferation rates
...
void AppLattice::iterate_kmc_global(double stoptime)
{
  ...
  while (!done) {
    ...
    if (isite >= 0) {
      ...
      int cd8aID = 0;
      int recID = 4;
      if(time + dt_step >= dt_diff*diffCount){
        int dCount = 0;
        for(int c=0; c < latCol*latCol; c++){
          dCount += population[3][c]; // count all dead cancer cells
        }
        nDead_mem.push_back({dCount, diffCount});
        if(diffCount == 0){
          int mCount = 0; 
          int cCount = 0;
          for(int c=0; c < latCol*latCol; c++){
            mCount += population[1][c]; // count all initial cancer cells
            cCount += population[0][c]; // count all initial cd8+
          }    
          nDead = cCount*mCount/4000; // prediction of number dead in 12 hour window BEFORE simulation begins
          nDeadi = nDead;
        }else if(0 < diffCount && diffCount < twindow){
          nDeadCounti += nDead_mem[nDead_mem.size() - 1][0];
          nDead = int(round((double(1 - double(diffCount/twindow))*nDeadi) + nDeadCounti));
        }else if(diffCount >= twindow){
          if(nDead_mem[nDead_mem.size() - 1][0] - nDead_mem[0][0] >= 0){
            nDead = nDead_mem[nDead_mem.size() - 1][0] - nDead_mem[0][0];
          }else{
            nDead = 0;
          }
          
          nDead_mem.erase(nDead_mem.begin());
        }
        diffCount++;
...
void AppLattice::iterate_kmc_sector(double stoptime)
{
  ...
  while (!alldone) {
    ...
    nsweeps++;
    // note melanoma ICS does not run in sector mode so this code is not complete for general use
   
    int latCol = 100;
    if(time >= dt_diff*diffCount){
      int dCount = 0;
      for(int c=0; c < latCol*latCol; c++){
        dCount += population[3][c]; // count all dead cancer cells
      }
      ...
      if(diffCount == 0){
        ...
        int cCount = 0;
        for(int c=0; c < latCol*latCol; c++){
          mCount += iarray[1][c]; // count all initial cancer cells
          cCount += iarray[0][c]; // count all initial cd8+
        }
        
        nDead = cCount*mCount / 5000;
      }else if(diffCount >= twindow){
        nDead = abs(nDead_mem[1][0] - nDead_mem[0][0]);
        nDead_mem.erase(nDead_mem.begin());
      }
      diffCount++;
    }
